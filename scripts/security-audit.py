...
for p in ROOT.rglob("*"):
 ...
 if b"PRIVATE KEY-----" in data:add("critical",rel,"private key material")
 ...
 if p.suffix in{".rs",".swift",".py",".sh",".toml",".json",".yml",".yaml"}:
  ...
  if re.search(r"(?i)(password|api[_-]?secret|private[_-]?key)\s*=\s*[\"'][^\"']+[\"']",text):add("high",rel,"hard-coded secret-shaped value")
...
report={"schema_version":1,"repository":"s1korrrr/crypto-inteligence","findings":findings,"counts":{s:sum(f["severity"]==s for f in findings)for s in["critical","high","medium","low"]},"status":"passed" if not any(f["severity"] in{"critical","high"}for f in findings)else"failed"}
