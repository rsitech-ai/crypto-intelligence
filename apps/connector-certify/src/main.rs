...
fn run(arguments: Arguments) -> Result<String, String> {
    if arguments.venue != "binance" {
        return Err("only the Binance venue is implemented".to_owned());
    }
...
fn workspace_root(fixtures: &Path) -> Result<PathBuf, String> {
    ...
    let expected = workspace
        .join("fixtures/exchanges/binance")
        .canonicalize()
    ...
    if provided != expected {
        return Err("fixture path is not the retained Binance fixture directory".to_owned());
    }
    Ok(workspace)
}
